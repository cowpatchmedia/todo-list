...
export default function createTaskForm(onSubmit = () => {}) {
  ...
  // -- Header Elements --
  const displayTitle = el('h3', 'modal-title-display', 'New Task');
  displayTitle.tabIndex = 0;
  ...
  ['low', 'medium', 'high', 'top'].forEach((val) => {
    const opt = el('option', '', val.charAt(0).toUpperCase() + val.slice(1), {
      value: val,
    });
  ...
  const addBtn = el('button', 'btn', 'Add', { type: 'submit' });
  ...
  const showDisplay = () => {
    displayTitle.textContent = titleInput.value.trim() || 'New Task';
    titleInput.style.display = 'none';
  ...
  const open = (initialData = null) => {
    if (initialData) {
      ...
      displayTitle.textContent = initialData.title || 'New Task';
